...
#### Pipeline 
Mainly the pipeline values for filtering were obtained by iteration and debugging to learn what worked and what did not. 

For the PassThrough Filter, multiple iterations were needed to obtain the values that would remove the table as much as possible without removing object pixels at the same time.
...
The Euclidean Clustering was also another part of the pipeline that needed iteration to obtain optimal values to perform object recognition. 
